/*
 ...
 Modifications were made by Jack Purdum and Dennis Kidder:
  ...
  W2ROW add receive offset - requires new hardware see Software Controlled Radio Yahoo Group
    Key in line to Arduino is pin D7
    Key out line from Arduino is D13
    RX_OFFSET can be set by user (currently -600 Hz)
...
#define KEY_IN_PIN        7               // W2ROW Key in pin
...
void setup() {
 ...
 lcd.backlight();                  //W2ROW
  ...
  sendFrequency(currentFrequency + RX_OFFSET);  //W2ROW Start in RX mode
...
void loop() {
  ...
  if (currentFrequency != oldFrequency) { // Are we still looking at the same frequency?
    ...
    sendFrequency(currentFrequency + RX_OFFSET); //W2ROW
